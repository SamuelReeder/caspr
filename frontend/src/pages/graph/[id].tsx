...
const GraphPage = () => {
      ...
      try {
        ...
        const graph = userGraphs.find((g: Graph) => {
          ...
          if (jsonData.timestamps) {
            ...
            const uniqueTimestamps: number[] = Array.from(new Set((jsonData.timestamps as { t: number }[]).map((timestamp) => timestamp.t))).sort((a, b) => a - b);
  ...
  const addDiagram = () => {
    ...
    const newTimestamp = diagrams.length ? diagrams[diagrams.length - 1].timestamp! + 1 : 1;
    const newData = parseTimestampedData(diagrams[0].data, newTimestamp);

    const newDiagram = {
      id: newId,
      timestamp: newTimestamp,
      data: newData,
      label: `${graph?.graphName} ${newTimestamp}`
    };
  ...
  const filteredData = diagrams[0]?.timestamp ? parseTimestampedData(diagrams[0].data, selectedTimestamp) : diagrams[0]?.data;
